...
def manifest(definitions):
    ...
    return {'contractVersion':'1.0','id':'c4isr.orbat-runtime','name':'ORBAT Runtime Static Plugin','version':'1.1.1','publisher':'Sutti','description':'Deterministic platform-neutral ORBAT symbol definitions and SVG renderer.','enabled':True,'requiredPermissions':['orbat.read'],'protocol':{'type':'static-assets','version':'1','entryPoint':'web/orbat-renderer.js'},'contributions':{'importConnectors':[{'id':'orbat.import.legacy-json','title':'Legacy ORBAT JSON (read-only adapter)','requiredPermission':'orbat.read','accepts':['.orbatsymbol.json','.orbatoverlay.json'],'target':'migration-preview'}],'symbolRenderers':[{'id':'renderer.orbat.svg.v1','title':'ORBAT deterministic SVG renderer','requiredPermission':'orbat.read','standard':'MIL-STD-2525','version':'1.1.0','renderer':'web/orbat-renderer.js','supportedAffiliations':['friend','hostile','neutral','unknown','suspect','civilian'],'supportedEchelons':['Team','Squad','Section','Platoon','Company','Battalion','Regiment','Brigade','Division','Corps','Army']}],'symbolTemplates':templates}}
def choose(defs,domain,role):return next((d for d in defs if d['domain']==domain and d['role']==role),None)
def fixtures(defs):
    lm=choose(defs,'land-unit','composite') or choose(defs,'land-unit','main-function')
    equipment_mains=list({d['definitionId']:d for d in defs if d['domain']=='equipment' and d['role']=='main-function'}.values())
    em=equipment_mains[0] if equipment_mains else choose(defs,'equipment','composite')
    equipment_function=equipment_mains[1] if len(equipment_mains)>1 else em
    variant=equipment_mains[2] if len(equipment_mains)>2 else em
    m1=choose(defs,'equipment','modifier-1') or choose(defs,'land-unit','modifier-1');m2=choose(defs,'equipment','modifier-2') or choose(defs,'land-unit','modifier-2');mob=choose(defs,'equipment','mobility-indicator');ech=choose(defs,'land-unit','echelon-indicator')
    if not lm or not em:raise RuntimeError('library lacks placeable land-unit/equipment definitions')
    cases=[]
    configs=[('land-friendly-present','land-unit','friend','present',lm,[('modifier1',m1),('modifier2',m2),('echelon',ech)]),('land-hostile-planned','land-unit','hostile','planned-anticipated',lm,[('modifier1',m1)]),('land-neutral-present','land-unit','neutral','present',lm,[]),('land-unknown-present','land-unit','unknown','present',lm,[('echelon',ech)]),('equipment-friendly-composed','equipment','friend','present',em,[('modifier1',m1),('modifier2',m2),('mobility',mob)]),('equipment-hostile-present','equipment','hostile','present',em,[('mobility',mob)]),('equipment-neutral-planned','equipment','neutral','planned-anticipated',em,[]),('equipment-unknown-present','equipment','unknown','present',em,[('modifier1',m1)]),('equipment-function-direct','equipment','friend','present',em,[('equipmentFunction',equipment_function)]),('equipment-variant-direct','equipment','friend','present',em,[('variant',variant)])]
    for affiliation in ('hostile','neutral','unknown'):
        configs.append((f'equipment-function-direct-{affiliation}','equipment',affiliation,'present',em,[('equipmentFunction',equipment_function)]))
        configs.append((f'equipment-variant-direct-{affiliation}','equipment',affiliation,'present',em,[('variant',variant)]))
